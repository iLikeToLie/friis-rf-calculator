...
const chartOptions = (xTitle: string) => ({
  ...
  scales: {
    x: { title: { display: true, text: xTitle, color: '#9c9c9d' }, grid: { color: 'rgba(255, 255, 255, 0.08)' }, ticks: { color: '#9c9c9d', maxTicksLimit: 12 } },
    y: { title: { display: true, text: 'Received power (dBm)', color: '#9c9c9d' }, grid: { color: 'rgba(255, 255, 255, 0.08)' }, ticks: { color: '#9c9c9d' } },
  },
  plugins: { legend: { labels: { color: '#cdcdcd', usePointStyle: true, pointStyle: 'line' as const } } },
});
  ...
  return (
    <div className="app-shell">
      <aside className="sidebar">
        ...
        <span className="version-chip">WEB 1.3 · BASED ON V3</span>
...
function DistanceView({ state, setState }: { state: AppState; setState: React.Dispatch<React.SetStateAction<AppState>> }) {
  ...
  const data = { labels: points.map((p) => p.distanceM), datasets: [
    { label: 'Received power', data: points.map((p) => p.receivedPowerDbm), borderColor: '#59d499', backgroundColor: 'rgba(89,212,153,.08)', fill: true, tension: .2, pointRadius: 0, borderWidth: 2.5 },
    { label: 'Receiver sensitivity', data: points.map(() => state.inputs.sensitivityDbm), borderColor: '#ffc533', borderDash: [7, 6], pointRadius: 0, borderWidth: 1.5 },
    { label: 'Interference threshold', data: points.map(() => state.inputs.interferenceThresholdDbm), borderColor: '#ff6161', borderDash: [7, 6], pointRadius: 0, borderWidth: 1.5 },
  ] };
...
function GainView({ state, setState }: { state: AppState; setState: React.Dispatch<React.SetStateAction<AppState>> }) {
  ...
  const colors = ['#59d499', '#57c1ff', '#a990ff'];
